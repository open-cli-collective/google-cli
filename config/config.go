...
// configScope is the cli-common state-scope for the registered CLI's config
// dir. The scope name is the CLI's `DirName`; the resolver returns the native
// per-OS user config dir (Linux $XDG_CONFIG_HOME or ~/.config; macOS
// ~/Library/Application Support; Windows %APPDATA%) plus that scope. A relative
// $XDG_CONFIG_HOME on Linux now yields an error (intentional tightening per
// cli-common/docs/working-with-state.md §1.1). It is a function, not a
// package var, so it reads DirName after Register has populated it rather than
// capturing an empty value at package-init time.
func configScope() statedir.Scope { return statedir.Scope{Name: DirName} }

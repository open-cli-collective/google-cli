// Package refresh implements `refresh` — the top-level cache
...
// today. Kept local to refresh (not exported from internal/cache) so the
...
// DriveLister is the narrow seam refresh needs from a Drive client — only
...
// NewCommand registers `refresh` with the production client factory.
...
func newCommandWithDeps(newClient ClientFactory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Refresh the local cache",
		Long: `Refresh the local cache of Google API metadata.

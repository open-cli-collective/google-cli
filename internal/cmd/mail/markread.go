...
Examples:
  mail mark-read msg123 msg456
  mail search "is:unread" --ids | mail mark-read --stdin
  mail mark-read --query "is:unread older_than:7d"`,
...
Examples:
  mail mark-unread msg123
  mail mark-unread --query "subject:important"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := newGmailClient(cmd.Context())
			if err != nil {
				return fmt.Errorf("creating Gmail client: %w", err)

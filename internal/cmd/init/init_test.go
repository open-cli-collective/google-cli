...
func TestInitCommand(t *testing.T) {
	...
	t.Run("Long describes the OAuth setup generically", func(t *testing.T) {
		t.Parallel()
		// The Long is intentionally identity-agnostic (rendered at
		// construction time, before config.Register): it must not hard-code a
		// specific CLI's API set. The concrete APIs are shown at runtime.

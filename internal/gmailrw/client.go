// Package gmailrw extends the shared read/organize Gmail client
// (google-cli-common/gmail) with the read-WRITE operations grw adds: trash and
...
// lives here, in grw, and never in the shared library.
package gmailrw
...
// machinery. The token must carry grw's scopes (see internal/appidentity);
// filters and permanent delete fail with a scope error otherwise.
func NewClient(ctx context.Context) (*Client, error) {
	base, err := gmail.NewClient(ctx)
	if err != nil {
		return nil, err
	}

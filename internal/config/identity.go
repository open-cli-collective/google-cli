...
// scope set and user-facing product name. It is the single seam that lets the
// internal infrastructure back multiple Google CLIs (e.g. gro, grw): each CLI process
// registers its identity exactly once at startup — from main, before any other
// config/keychain/auth call — so the application resolves the correct config/cache
...
// (and for any test helper that needs a valid identity without

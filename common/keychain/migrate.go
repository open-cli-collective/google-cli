...
// "service" attribute) — the registered CLI's DirName. The account/key was
// already "oauth_token", so unlike slck there is no field rename. It is a
// function, not a const, because DirName is populated by config.Register at
// startup rather than at compile time.
func legacyKeychainService() string { return config.DirName }
...
func migrateLegacyOverwrite(s *Store, cfg *config.Config, overwrite bool) error {
	...
	if plan.signal {
		human := fmt.Sprintf("%s: migrated %s into keyring %s (from %s; legacy original removed)",
			config.ProductName(), plan.change.Field, s.ref, plan.change.From)
...
func conflictErr(service, profile, ref string, cands []candidate, hasTarget bool) error {
	...
	return credstore.MigrationConflictError(config.ProductName(), "oauth_token", strings.Join(locs, ", "), ref)
}

...
Examples:
  mail label "Work" msg123 msg456
  mail search "from:boss" --ids | mail label "Important" --stdin
  mail label "Projects" --query "subject:sprint"`,
...
Examples:
  mail unlabel "Work" msg123
  mail unlabel "Old" --query "label:Old older_than:90d"`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			labelName := args[0]
			messageArgs := args[1:]

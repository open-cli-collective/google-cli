...
// and reusing its authenticated service and label cache, without this package
// itself taking on any destructive surface. google-readonly never calls it.
func (c *Client) Service() *gmail.Service { return c.service }

// UserID returns the account selector used for API calls (always "me").
func (c *Client) UserID() string { return c.userID }

// InvalidateLabels clears the cached label set so the next FetchLabels re-reads
// from the API. Callers that mutate labels (create/rename/delete) use it to keep
// the name<->ID cache coherent within a single process.
func (c *Client) InvalidateLabels() {
	c.labelsMu.Lock()
	c.labels = nil
	c.labelsByName = nil

// Package setcred implements `set-credential` — the low-level,
...
// NewCmd builds the `set-credential` command.
...
flag or positional argument. Only the key 'oauth_token' is accepted.
  ...
  op read 'op://Vault/gro/oauth_token' | set-credential --key oauth_token --stdin
  set-credential --key oauth_token --from-env GRO_OAUTH_TOKEN`,
		Args: cobra.NoArgs,
